...
public class AddRecipeFragment extends Fragment {
    ...
    private List<Ingredient> collectIngredients() {
        ...
        for (int i = 0; i < ingredientsContainer.getChildCount(); i++) {
            ...
            String quantity = quantityEditText != null ? Objects.requireNonNull(quantityEditText.getText()).toString().trim() : "";
            String unit = unitEditText != null ? Objects.requireNonNull(unitEditText.getText()).toString().trim() : "";

            Ingredient ingredient = new Ingredient();
            ingredient.name = name;
            ingredient.quantity = quantity + " " + unit;  // Łączenie ilości z jednostką
